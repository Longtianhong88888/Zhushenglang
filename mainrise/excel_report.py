...
def write_tracking_excel(path: Path, date: str, buy_points: pd.DataFrame,
    ...
    if found.empty:
        ...
        rows = [[r["code"], r["name"], "B3打底仓" if r["kind"] == "B3" else "二波加仓",
    ...
    rules = [
        ("第一级 B3", "均线粘合≤3% + 放量阳线(量比≥2) + 站上5/10/20日均线 + 距60日低点<30% → 打底仓"),
        ("第二级 二波", "B3后3~30日深回调2-12% + 均线再次粘合≤2% + 缩量 → 放量再启动 → 加仓"),
        ("止损", "买入价 -4%，或跌破 MA10"),
        ("止盈", "高点回落 8%（条件单）"),
        ("时间止损", "持仓 10-20 个交易日（让赢家跑）"),
